...
int oplus_display_read_serial_number(struct dsi_display *display, unsigned long *serial_number)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if (display->panel->oplus_panel.serial_number.is_multi_reg) {
			for (j = 0; j < display->panel->oplus_panel.serial_number.serial_number_conut; j++) {
				ret = dsi_panel_read_panel_reg_unlock(m_ctrl, display->panel, display->panel->oplus_panel.serial_number.serial_number_multi_regs[j],
					&read[j], 1);
				if (ret < 0) {
