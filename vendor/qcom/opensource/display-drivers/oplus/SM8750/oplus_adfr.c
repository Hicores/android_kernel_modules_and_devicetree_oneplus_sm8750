...
/* extern functions */
void _get_tearcheck_cfg(struct sde_encoder_phys *phys_enc,
...
int oplus_adfr_property_update(void *sde_connector, void *sde_connector_state, int prop_id, uint64_t prop_val)
{
	...
	case CONNECTOR_PROP_ADFR_MIN_FPS:
		...
		ADFR_DEBUG("CONNECTOR_PROP_ADFR_MIN_FPS:0x%08llx\n", prop_val);
		...
		if (prop_val & OPLUS_ADFR_SA_MAGIC) {
			...
			if (prop_val & OPLUS_ADFR_AUTO_MODE_MAGIC) {
				if (p_oplus_adfr_params->need_filter_auto_on_cmd
					...
					if (prop_val & OPLUS_ADFR_SA_MIN_FPS_MAGIC) {
						...
						ADFR_ERR("failed to update sw fps because of the sa min fps magic error,prop_val=0x%08llx\n", prop_val);
					}
					handled |= BIT(2);
					goto end;
			...
			/* latest setting */
			ADFR_INFO("auto_mode:%u[%d],sa_min_fps:%u[%d],sw_fps:%u,handled:0x%02x\n",
...
int oplus_adfr_irq_handler(void *sde_encoder_phys, unsigned int irq_type)
{
	...
	if (irq_type == OPLUS_ADFR_RD_PTR) {
		...
		ADFR_DEBUG("rd_ptr_timestamp_us:%lu, last_rd_ptr_timestamp_us:%lu\n", rd_ptr_timestamp_us, last_rd_ptr_timestamp_us);
		...
		ADFR_DEBUG("oplus_adfr_osync_frame_status:%u\n", p_oplus_adfr_params->osync_frame_status);
		OPLUS_ADFR_TRACE_INT("oplus_adfr_osync_frame_status", p_oplus_adfr_params->osync_frame_status);
		ADFR_DEBUG("wr_ptr_timestamp_us:%lu, last_wr_ptr_timestamp_us:%lu\n", wr_ptr_timestamp_us, last_wr_ptr_timestamp_us);
...
/* min fps cmd will be reset in some case, so set sa_min_fps_updated flag, then sa_min_fps will be updated in oplus_adfr_sa_handle */
int oplus_adfr_set_min_fps_updated(void *dsi_panel)
{
	struct dsi_panel *panel = dsi_panel;
	struct oplus_adfr_params *p_oplus_adfr_params = NULL;
...
#if defined(CONFIG_PXLW_IRIS)
	if (iris_is_chip_supported() && (!strcmp(panel->type, "secondary"))) {
		ADFR_INFO("no need to set min fps updated for iris chip\n");
		return 0;
	}
#endif /* CONFIG_PXLW_IRIS */
...
int oplus_adfr_status_reset(void *dsi_panel)
{
	...
	if ((h_skew == STANDARD_ADFR) || (h_skew == STANDARD_MFR)) {
		...
		if (oplus_adfr_high_precision_sa_mode_is_enabled(p_oplus_adfr_params)) {
			...
			ADFR_INFO("sa status reset: auto_mode:%u,sa_min_fps:%u,sa_high_precision_fps:%u\n",
					p_oplus_adfr_params->auto_mode, p_oplus_adfr_params->sa_min_fps, p_oplus_adfr_params->sa_high_precision_fps);
		} else {
			ADFR_INFO("sa status reset: auto_mode:%u,sa_min_fps:%u\n",
...
int oplus_adfr_test_te_high_gear(void *dsi_display) {
...
int oplus_adfr_test_te_low_gear(void *dsi_display) {
	struct dsi_display *display = (struct dsi_display *)dsi_display;
	struct dsi_display_mode *mode = NULL;
	struct oplus_adfr_params *p_oplus_adfr_params = NULL;
	int low_gear = 16;
...
int oplus_adfr_test_te_middle_value(void *dsi_display) {
	struct dsi_display *display = (struct dsi_display *)dsi_display;
	struct dsi_display_mode *mode = NULL;
	struct oplus_adfr_params *p_oplus_adfr_params = NULL;
	int middle_value = 30;
...
static irqreturn_t oplus_adfr_test_te_irq_handler(int irq, void *data)
{
	...
	if (p_oplus_adfr_params->test_te.config != OPLUS_ADFR_TEST_TE_DISABLE) {
		...
		if ((mode->timing.h_skew == STANDARD_ADFR) || (mode->timing.h_skew == STANDARD_MFR)) {
			if (!oplus_adfr_decreasing_step_is_enabled(p_oplus_adfr_params)) {
				 ...
				 * 60hz: 17~45 is 30 */
					...
					p_oplus_adfr_params->test_te.refresh_rate = oplus_adfr_test_te_middle_value(display);
				/* range:
				 * 144hz: X
		...
		if (p_oplus_adfr_params->test_te.config == OPLUS_ADFR_TEST_TE_ENABLE_WITCH_LOG) {
			...
			if ((mode->timing.h_skew == STANDARD_ADFR) || (mode->timing.h_skew == STANDARD_MFR)) {
				ADFR_INFO("h_active:%u,v_active:%u,fps:%u,h_skew:%u,auto_mode:%u,sa_min_fps:%u,sw_fps:%u,idle_mode:%u\n",
		...
		if (c_conn->qsync_mode != SDE_RM_QSYNC_DISABLED) {
			...
			if (!(prop_val & OPLUS_ADFR_SA_MAGIC)) {
				if (prop_val != p_oplus_adfr_params->osync_min_fps) {
					ADFR_INFO("updated osync min fps %u -> %llu\n",
	...
	if (oplus_adfr_high_precision_oa_mode_is_enabled(p_oplus_adfr_params)) {
		if ((h_skew == OPLUS_ADFR) && (refresh_rate == 120)) {
			...
			if ((priv_info->oplus_adfr_sw_stabilize_frame_config_table_count)
				...
				&& (oplus_panel_pwm_get_state(display->panel) == PWM_STATE_L2)
				&& p_oplus_adfr_params->osync_min_fps) {
...
#if defined(CONFIG_PXLW_IRIS)
	if (iris_is_chip_supported() && (!strcmp(display->display_type, "secondary"))) {
		ADFR_ERR("can not set min fps for iris chip\n");
		return count;
	}
#endif /* CONFIG_PXLW_IRIS */
...
#if defined(CONFIG_PXLW_IRIS)
	if (iris_is_chip_supported() && (!strcmp(display->display_type, "secondary"))) {
		ADFR_ERR("can not get min fps refresh rate for iris chip\n");
		return -EFAULT;
	}
#endif /* CONFIG_PXLW_IRIS */
