...
static void get_oplus_project_value(const char *key) {
	int ret;
	size_t length = 0;
	char *buffer;
	...
	if (key == NULL || strlen(key) == 0)	{
		pr_err("get_oplus_project_value key is null\n");
		return;
	}
	...
	if (strcmp(key, OPLUS_WIFI_NAME_RFID) == 0) {
		...
		pr_err("get_oplus_project_value key: %s ret: %d\n", key, ret);
		if (ret >= 0) {
			memset(buffer, 0, length);
			ret = snprintf(buffer, length, "%d", ret);
			if (ret >= length) {
				pr_err("get_oplus_project_value key: %s ret: %d, value is too long, reset buffer for error!\n", key, ret);
				memset(buffer, 0, length);
			} else {
				pr_err("get_oplus_project_value key: %s buffer: %s successfully\n", key, buffer);
			}
		}
		return;
	}
...
static const char* get_dts_info(struct wifi_data *wifi_data, const char* dts_type, const char* dts_name) {
	...
	while (node) {
		...
		if(project_type) {
			get_oplus_project_value(project_type);
