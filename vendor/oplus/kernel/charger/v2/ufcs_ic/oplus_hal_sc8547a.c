...
static bool sc8547a_hw_version_check(struct sc8547a_device *chip)
{
	...
	if (val == SC8547A_DEVICE_ID) {
		ic_sc8547a = true;
		chg_info("device id is sc8547a \n");
		return true;
	} else {
		ic_sc8547a = false;
		chg_info("device id is sc8547 \n");
		return false;
	}
}
...
static int sc8547_voocphy_reset_voocphy(struct oplus_voocphy_manager *chip)
{
	...
	sc8547_write_byte(chip->client, SC8547_REG_02, 0x01); /*VAC_OVP:12*/
...
static int sc8547_init_device(struct sc8547a_device *chip)
{
	...
	sc8547_write_byte(chip->client, SC8547_REG_02, 0x1); /*VAC_OVP:12*/
...
static int sc8547a_ufcs_cp_watchdog_config(struct ufcs_dev *ufcs, unsigned int time_ms)
{
	struct sc8547a_device *chip = ufcs->drv_data;
	unsigned int en = 0;
	int rc;
	...
	if (time_ms == 0) {
		en = SC8547_WATCHDOG_DIS;
	} else if (time_ms > 0 && time_ms <= 200) {
		en = BIT(0);
	} else if (time_ms > 200 && time_ms <= 500) {
		en = BIT(1);
	} else if (time_ms > 500 && time_ms <= 1000) {
		en = BIT(1) | BIT(0);
	} else if (time_ms > 1000 && time_ms <= 5000) {
		en = BIT(2);
	} else if (time_ms > 5000 && time_ms <= 30000) {
		en = BIT(2) | BIT(0);
	} else {
		chg_err("time_ms set error (%d)", time_ms);
		return -EINVAL;
	}
...
static void sc8547a_check_fault_into(struct sc8547a_device *chip)
{
	...
	if (buf[1] & SC8547A_IBUS_OCP_FLAG_MASK) {
		chg_err("ibus ocp\n");
		vote(chip->disable_votable, OCP_VOTER, chip->work_start, 1, false);
		oplus_chg_ic_creat_err_msg(chip->cp_ic, OPLUS_IC_ERR_CP,
...
#define SC8547A_CP_STATUS_REG_MAX	4
static u8 g_sc8547a_cp_status_reg[SC8547A_CP_STATUS_REG_MAX] = {
	0x06, 0x09, 0x0e, 0x0f
};

static void sc8547a_cp_regdump_work(struct work_struct *work)
{
	struct sc8547a_device *chip =
		container_of(work, struct sc8547a_device, cp_regdump_work);
	char *buf;
	int i;
	size_t index = 0;
	u8 data;
	...
	for (i = 0; i < SC8547A_CP_STATUS_REG_MAX; i++) {
		data = 0;
		sc8547a_read_data(chip, g_sc8547a_cp_status_reg[i], &data, 1);
		index += snprintf(buf + index, ERR_MSG_BUF, "0x%02x=%02x,",
			g_sc8547a_cp_status_reg[i], data);
	}
	if (index > 0)
		buf[index - 1] = 0;
...
static int sc8547a_get_wdt_reg_by_time(unsigned int time_ms, u8 *reg)
{
	if (time_ms == 0) {
...
static int sc8547a_cp_wd_enable(struct oplus_chg_ic_dev *ic_dev, int timeout_ms)
{
	struct sc8547a_device *chip;
	u8 reg_val = 0;
	int ret = 0;
	...
	ret = sc8547a_get_wdt_reg_by_time(timeout_ms, &reg_val);
	if (ret < 0)
		return ret;
...
static int sc8547a_cp_watchdog_reset(struct oplus_chg_ic_dev *ic_dev)
{
	struct sc8547a_device *chip;
	u8 data;
	int rc;
...
static void *sc8547a_cp_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_CP_WATCHDOG_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_CP_WATCHDOG_ENABLE, sc8547a_cp_wd_enable);
		break;
...
static void sc8547a_ic_offline_work(struct work_struct *work)
{
	struct sc8547a_device *chip =
		container_of(work, struct sc8547a_device, ic_offline_work);

	if (!chip->work_start)
		return;
	sc8547a_cp_set_work_start(chip->cp_ic, false);
	sc8547a_cp_enable(chip->cp_ic, false);
}

static int sc8547a_disable_vote_callback(struct votable *votable, void *data,
					 int disable, const char *client, bool step)
{
	struct sc8547a_device *chip = data;
	bool online;
	...
	if (disable < 0)
		...
		online = !disable;
	chg_info("[%s]%s set sc8547a online to %s\n", votable_name(votable),
	...
	if (online) {
		...
		/*
		* The first online operation during the startup phase
		* does not require reporting of exceptions
		*/
		if (likely(chip->boot_online)) {
			oplus_chg_ic_creat_err_msg(chip->cp_ic, OPLUS_IC_ERR_CP,
						   CP_ERR_ONLINE_CHANGE,
						   "online");
			oplus_chg_ic_err_trigger_and_clean(chip->cp_ic);
		} else {
			chip->boot_online = true;
		}
	} else {
	...
	for_each_child_of_node(chip->dev->of_node, child) {
		...
		case OPLUS_CHG_IC_CP:
			...
			snprintf(votable_name, CP_IC_VOTEABLE_NAME_MAX - 1,
				 "SC8547A_DISABLE:%d", ic_index);
			chip->disable_votable = create_votable(votable_name,
				VOTE_SET_ANY, sc8547a_disable_vote_callback, chip);
...
#endif
{
	...
	INIT_WORK(&chip->cp_regdump_work, sc8547a_cp_regdump_work);
	INIT_WORK(&chip->ic_offline_work, sc8547a_ic_offline_work);
...
static void sc8547a_ufcs_shutdown(struct i2c_client *client)
{
...
static const struct of_device_id sc8547a_ufcs_match[] = {
	{.compatible = "oplus,sc8547a" },
	{},
};

static const struct i2c_device_id sc8547a_ufcs_id[] = {
	{ "oplus,sc8547a", 0 },
	{},
};
MODULE_DEVICE_TABLE(i2c, sc8547a_ufcs_id);

static struct i2c_driver sc8547a_ufcs_i2c_driver = {
	.driver =
		{
			.name = "sc8547a",
			.owner = THIS_MODULE,
			.of_match_table = sc8547a_ufcs_match,
		},
	.probe = sc8547a_driver_probe,
	.id_table = sc8547a_ufcs_id,
	.shutdown = sc8547a_ufcs_shutdown,
};

#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0))
static int __init sc8547a_ufcs_i2c_driver_init(void)
{
	int ret = 0;
	chg_info(" init start\n");

	if (i2c_add_driver(&sc8547a_ufcs_i2c_driver) != 0) {
...
subsys_initcall(sc8547a_ufcs_i2c_driver_init);
#else
static __init int sc8547a_ufcs_i2c_driver_init(void)
{
	return i2c_add_driver(&sc8547a_ufcs_i2c_driver);
}

static __exit void sc8547a_ufcs_i2c_driver_exit(void)
{
	i2c_del_driver(&sc8547a_ufcs_i2c_driver);
}

oplus_chg_module_register(sc8547a_ufcs_i2c_driver);
