...
#define pr_fmt(fmt) "[MTK6991]([%s][%d]): " fmt, __func__, __LINE__
...
#define OTG_BOOST_VOL_THRESHOLD		    5000
...
extern int mt6379_get_hvdcp_type(void);
extern void mt6379_enable_hvdcp_detect(void);
extern int mt6379_set_hvdcp_to_5v(void);
extern int mt6379_set_hvdcp_to_9v(void);
...
#endif
extern bool mt6379_int_chrdet_attach(void);
#endif
...
bool oplus_mt_get_vbus_status(void)
{
	...
	if (pinfo->chrdet_state)
		return true;
	else if (mt6379_int_chrdet_attach() == true)
		return true;
	else
		return false;
}
				...
				struct device *dev)
{
	...
	info->usb_aicl_enhance = of_property_read_bool(np, "oplus,usb_aicl_enhance");
	chr_info("usb_aicl_enhance:%d", info->usb_aicl_enhance);
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_CABLE_TYPE:
		if (noti->cable_type.type == TCPC_CABLE_TYPE_A2C) {
			oplus_chg_ic_virq_trigger(pinfo->ic_dev, OPLUS_IC_VIRQ_PD_COMPLETED);
			chr_err("CABLE_TYPE = A to C\n");
		}
		break;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0))
...
static int oplus_mt6379_suspend_charger(struct mtk_charger *info)
{
...
static int oplus_mt6379_unsuspend_charger(struct mtk_charger *info)
{
...
static int oplus_mt6379_enable_charging(struct mtk_charger *info)
{
	int rc = 0;
...
static int oplus_mt6379_disable_charging(struct mtk_charger *info)
{
	int rc = 0;
...
static int oplus_mt6379_check_charging_enable(struct mtk_charger *info)
{
	int rc = 0;
	bool enable = false;
...
static int oplus_mt6379_set_aicl_point(struct oplus_chg_ic_dev *ic_dev, int vbatt)
{
	int rc = 0;
...
static int oplus_mt6379_input_current_limit_write_without_aicl(struct mtk_charger *info,
			int value)
{
	struct charger_device *chg = NULL;
	int rc = 0;
...
static int oplus_chg_usb_set_input_current(struct mtk_charger *info, int current_ma,
	int aicl_point)
{
	int rc = 0, i = 0;
	int chg_vol = 0;
	...
	for (i = 1; i <= current_ma / 100; i++) {
		rc = charger_dev_set_input_current(chg, i * 1000 * 100);
		if (rc) {
			chg_err("set icl to %d uA fail, rc=%d\n", i * 100000, rc);
			return rc;
		} else {
			chg_err("set icl to %d uA\n", i * 100000);
		}
		msleep(90);
		chg_vol = battery_meter_get_charger_voltage();
...
static int oplus_mt6379_input_current_limit_write(struct mtk_charger *info, int value)
{
	int rc = 0;
	int i = 0;
	int chg_vol = 0;
	...
	if (info->usb_aicl_enhance) {
		...
		if (rc >= 0  && (charger_type == OPLUS_CHG_USB_TYPE_SDP ||
			...
			if (charger_type == OPLUS_CHG_USB_TYPE_SDP) {
				...
				oplus_mt6379_set_aicl_point(info->ic_dev, batt_volt);
			}
...
static int oplus_mt6379_charging_current_write_fast(struct mtk_charger *info, int chg_curr)
{
	int rc = 0;
	u32 ret_chg_curr = 0;
...
static int oplus_mt6379_float_voltage_write(struct mtk_charger *info, int vfloat_mv)
{
	int rc = 0;
...
static int oplus_mt6379_float_voltage_read(struct mtk_charger *info)
{
	int rc = 0;
	int vfloat_mv = 0;
...
static int oplus_mt6379_set_termchg_current(struct mtk_charger *info, int term_curr)
{
	int rc = 0;
...
static int oplus_mt6379_get_chg_ibus(struct mtk_charger *info)
{
	u32 ibus = 0;
	int ret = 0;
...
static int oplus_mt6379_otg_enable(struct mtk_charger *info)
{
	return charger_dev_enable_otg(info->chg1_dev, true);
}

static int oplus_mt6379_otg_disable(struct mtk_charger *info)
{
	return charger_dev_enable_otg(info->chg1_dev, false);
}

static int oplus_mt6379_set_chging_term_disable(struct mtk_charger *info)
{
	int rc = 0;
...
static int mtk_chg_input_suspend(struct oplus_chg_ic_dev *ic_dev, bool suspend)
{
	...
	if (suspend)
		rc = oplus_mt6379_suspend_charger(info);
	else
		rc = oplus_mt6379_unsuspend_charger(info);
...
static int mtk_chg_output_suspend(struct oplus_chg_ic_dev *ic_dev, bool suspend)
{
	...
	if (suspend)
		rc = oplus_mt6379_disable_charging(info);
	else
		rc = oplus_mt6379_enable_charging(info);
...
static int mtk_chg_output_is_suspend(struct oplus_chg_ic_dev *ic_dev, bool *suspend)
{
	...
	*suspend = oplus_mt6379_check_charging_enable(info);
			   ...
			   bool step, int icl_ma)
{
	...
	if (step)
		return oplus_mt6379_input_current_limit_write(info, icl_ma);
	else
		return oplus_mt6379_input_current_limit_write_without_aicl(info, icl_ma);
}
...
static int mtk_chg_set_fcc(struct oplus_chg_ic_dev *ic_dev, int fcc_ma)
{
	...
	return oplus_mt6379_charging_current_write_fast(info, fcc_ma);
}
...
static int mtk_chg_set_fv(struct oplus_chg_ic_dev *ic_dev, int fv_mv)
{
	...
	return oplus_mt6379_float_voltage_write(info, fv_mv);
}
...
static int mtk_chg_get_fv(struct oplus_chg_ic_dev *ic_dev, int *fv_mv)
{
	...
	*fv_mv = oplus_mt6379_float_voltage_read(info);
...
static int mtk_chg_set_iterm(struct oplus_chg_ic_dev *ic_dev, int iterm_ma)
{
	...
	return oplus_mt6379_set_termchg_current(info, iterm_ma);
}
...
static int mtk_chg_get_input_curr(struct oplus_chg_ic_dev *ic_dev, int *curr_ma)
{
	...
	*curr_ma = oplus_mt6379_get_chg_ibus(info) / 1000;
...
static int mtk_chg_otg_boost_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	...
	if (en)
		rc = oplus_mt6379_otg_enable(info);
	else
		rc = oplus_mt6379_otg_disable(info);
	if (rc < 0)
...
static int mtk_chg_set_otg_boost_curr_limit(struct oplus_chg_ic_dev *ic_dev, int curr_uA)
{
	...
	if ((batt_soc <= LOW_BATT_SOC)
		&& (info->low_batt_otg_boost_curr_ua != 0) && (info->low_batt_otg_boost_curr_ua <= curr_uA)) {
...
#ifdef CONFIG_OPLUS_HVDCP_SUPPORT
	if (mt6379_get_hvdcp_type() == POWER_SUPPLY_TYPE_USB_HVDCP)
...
static void oplus_chg_set_qc_config_forvoocphy(struct mtk_charger *chip, int vol_mv)
{
	...
	if (vol_mv == 9000) {
		...
		oplus_mt6379_set_aicl_point(chip->ic_dev, batt_volt);
	} else {
		if (chg_vol > OPLUS_CHG_PULL_UP_VOLT_THR) {
			mtk_chg_aicl_reset(chip->ic_dev);
			mt6379_set_hvdcp_to_5v();
			msleep(PDQC_BOOST_DELAY_MS);
			if (is_gauge_topic_available(chip)) {
				oplus_mms_get_item_data(chip->gauge_topic,
						GAUGE_ITEM_VOL_MAX, &data, false);
...
static int mtk_chg_should_disable_pd(struct oplus_chg_ic_dev *ic_dev)
{
	...
	chg_info("pd_svooc = %d, vooc_disable = %d\n", chip->pd_svooc, vooc_disable);
...
static int mtk_chg_set_pd_config(struct oplus_chg_ic_dev *ic_dev, u32 pdo)
{
	...
	if (mtk_chg_should_disable_pd(ic_dev)) {
		chg_info("disable pd");
		return 0;
	}
...
static struct mtk_gauge* oplus_gauge_get_mtk_gauge(void)
{
	static struct mtk_gauge *gauge = NULL;
	struct power_supply *psy;
...
static int oplus_gauge_get_battery_temp_adc_voltage(int *val)
{
	int ret = 0;
	static struct mtk_gauge *gauge = NULL;

	gauge = oplus_gauge_get_mtk_gauge();
	if (NULL == gauge) {
		chg_err("gauge is NULL.");
		return -EOPNOTSUPP;
	}

	if (IS_ERR(gauge->chan_bat_temp)) {
		chg_err("auxadc chan: baton(bat_temp) error\n");
		return -EOPNOTSUPP;
	}
	...
	chg_debug("bat_temp_volt = %d, ret = %d", *val, ret);

	return ret;
}

static int oplus_get_sub_btb_state(struct mtk_charger *info)
{
	int adc_voltage = 0;
	...
	if (info->sub_btb_valid_adc[0] || info->sub_btb_valid_adc[1]) {
		/* get the real batt temp adc voltage */
		rc = oplus_gauge_get_battery_temp_adc_voltage(&adc_voltage);
		if (rc < 0) {
		...
		if (info->sub_btb_valid_adc[0] && !info->sub_btb_valid_adc[1]) {
			/*
			 * for silicon battery, when the valid temp_adc is less than 0.5V, the sub_btb is connected,
			 * otherwise, the sub_btb is disconnected.
			 */
			if (adc_voltage >= info->sub_btb_valid_adc[0])
				state = BATT_BTB_STATE_NOT_CONNECT;
			else
				state = BATT_BTB_STATE_CONNECT;
		}
	}
	chg_debug("adc_voltage = %d, adc_valid[%d - %d], state = %d",
		  adc_voltage, info->sub_btb_valid_adc[0],
		  info->sub_btb_valid_adc[1], state);
...
static void *oplus_chg_mt6991_gauge_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	void *func = NULL;
...
static int mtk_chg_get_usb_aicl_enhance(struct oplus_chg_ic_dev *ic_dev, bool *enable)
{
	struct mtk_charger *info;
				...
				enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_BUCK_GET_USB_AICL_ENHANCE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_USB_AICL_ENHANCE,
					       mtk_chg_get_usb_aicl_enhance);
		break;
	default:
		chg_err("this func(=%d) is not supported\n", func_id);
		func = NULL;
...
struct oplus_chg_ic_virq oplus_chg_mt6991_gauge_virq_table[] = {
	{ .virq_id = OPLUS_IC_VIRQ_ERR },
};
#endif /* OPLUS_FEATURE_CHG_BASIC */
...
static int oplus_chg_get_usb_btb_temp_cal(void)
{
	...
	pinfo->i_sub_board_temp = subboard_temp / 10;/*bringup 6991*/
	chg_info("subboard_temp:%d,volt:%d\n", pinfo->i_sub_board_temp, ntc_param.ui_dwvolt);
	return pinfo->i_sub_board_temp;
}
...
int oplus_mt6379_get_tchg(int *tchg_min,	int *tchg_max)
{
	if (pinfo != NULL) {
...
static int oplus_mtk_ic_register(struct device *dev, struct mtk_charger *info)
{
	...
	snprintf(ic_cfg.manu_name, OPLUS_CHG_IC_MANU_NAME_MAX - 1, "buck-MTK6991");
	...
	for_each_child_of_node(dev->of_node, child) {
		...
		case OPLUS_CHG_IC_GAUGE:
			snprintf(ic_cfg.manu_name, OPLUS_CHG_IC_MANU_NAME_MAX - 1, "gauge-MTK6991");
			snprintf(ic_cfg.fw_id, OPLUS_CHG_IC_FW_ID_MAX - 1, "0x00");
			ic_cfg.get_func = oplus_chg_mt6991_gauge_get_func;
			ic_cfg.virq_data = oplus_chg_mt6991_gauge_virq_table;
			ic_cfg.virq_num = ARRAY_SIZE(oplus_chg_mt6991_gauge_virq_table);
			break;
		case OPLUS_CHG_IC_PPS:
			snprintf(ic_cfg.manu_name, OPLUS_CHG_IC_MANU_NAME_MAX - 1, "pps-MTK6991");
			snprintf(ic_cfg.fw_id, OPLUS_CHG_IC_FW_ID_MAX - 1, "0x00");
...
#define OPLUS_PD_9V_PDO 0x32d12c
	...
	if (vol_mv == VBUS_5V)
		rc = mtk_chg_set_pd_config(ic_dev, OPLUS_PD_5V_PDO);
	else if (vol_mv == VBUS_9V)
		rc = mtk_chg_set_pd_config(ic_dev, OPLUS_PD_9V_PDO);
	else
		return -1;
...
#define WLS_CHG_CHECK_DELAY	msecs_to_jiffies(500)
#define WLS_CHG_CHECK_INTERVAL	msecs_to_jiffies(5000)
static void oplus_wls_chg_check_work(struct work_struct *work)
{
	struct mtk_charger *chip = container_of(work, struct mtk_charger, wls_chg_check_work.work);
	int rc;
	bool chg_en = false;

	if (!chip->wls_online) {
		chg_info("wls is not online\n");
		return;
	}

	if (chip->wls_charging_disable) {
		rc = charger_dev_is_enabled(chip->chg1_dev, &chg_en);
		if (rc >= 0 && chg_en) {
			chg_info("disable charging...\n");
			charger_dev_enable(chip->chg1_dev, false);
		}
	}

	schedule_delayed_work(&chip->wls_chg_check_work, WLS_CHG_CHECK_INTERVAL);
}

static void oplus_mt6379_wls_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id, bool sync)
{
	...
	case MSG_TYPE_ITEM:
		...
		case WLS_ITEM_PRESENT:
			...
			if (!chip->wls_online)
				cancel_delayed_work(&chip->wls_chg_check_work);
			break;
		case WLS_ITEM_CHARGING_DISABLE:
			oplus_mms_get_item_data(chip->wls_topic, id, &data, false);
			chip->wls_charging_disable = !!data.intval;
			if (chip->wls_charging_disable)
				schedule_delayed_work(&chip->wls_chg_check_work, WLS_CHG_CHECK_DELAY);
			else
				cancel_delayed_work(&chip->wls_chg_check_work);
			break;
		default:
...
static void oplus_mt6379_subscribe_wls_topic(struct oplus_mms *topic, void *prv_data)
{
	struct mtk_charger *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->wls_topic = topic;
	chip->wls_subs = oplus_mms_subscribe(chip->wls_topic, chip, oplus_mt6379_wls_subs_callback, "mt6379");
...
static int subborad_thermal_read_temp(struct thermal_zone_device *tzd, int *temp)
{
	if (temp != NULL)
		*temp = oplus_get_subboard_temp() * 100; /* thermal zone need 0.001 */

	return 0;
}

static struct thermal_zone_device_ops subboard_temp_tzd_ops = {
	.get_temp = subborad_thermal_read_temp,
};

static int bat_btb_thermal_read_temp(struct thermal_zone_device *tzd, int *temp)
{
	if (temp != NULL)
		*temp = oplus_chg_get_battery_btb_temp_cal() * 100; /* thermal zone need 0.001 */

	return 0;
}

static struct thermal_zone_device_ops bat_btb_temp_tzd_ops = {
	.get_temp = bat_btb_thermal_read_temp,
};

static int bat_sub_btb_thermal_read_temp(struct thermal_zone_device *tzd, int *temp)
{
	if (temp != NULL)
		*temp = oplus_chg_get_sub_battery_btb_temp_cal() * 100; /* thermal zone need 0.001 */

	return 0;
}

static struct thermal_zone_device_ops bat_sub_btb_temp_tzd_ops = {
	.get_temp = bat_sub_btb_thermal_read_temp,
};
...
static int register_tz_thermal(struct mtk_charger *info)
{
	...
	info->master_cp_temp_tzd = register_tz_device("master_cp_temp", &master_cp_temp_tzd_ops);
	info->slave_cp_temp_tzd = register_tz_device("slave_cp_temp", &slave_cp_temp_tzd_ops);
	info->subboard_temp_tzd = register_tz_device("subboard_temp", &subboard_temp_tzd_ops);
	info->bat_btb_temp_tzd = register_tz_device("bat_btb_temp", &bat_btb_temp_tzd_ops);
	info->bat_sub_btb_temp_tzd = register_tz_device("bat_sub_btb_temp", &bat_sub_btb_temp_tzd_ops);

	return 0;
}
#endif

static int oplus_gauge_sub_btb_parse_dt(struct mtk_charger *chip, struct device *dev)
{
	int rc = 0;
	int i = 0;
	...
	rc = read_signed_data_from_node(dev->of_node, "oplus,sub_btb_valid_temp",
					(s32 *)(chip->sub_btb_valid_adc),
					OPLUS_SUB_BTB_MAX);
	if (rc < 0) {
		chg_err("get sub_btb_valid_temp, rc = %d\n", rc);
		for (i = 0; i < OPLUS_SUB_BTB_MAX - 1; i++)
			chip->sub_btb_valid_adc[i] = 0;
	} else {
		for (i = 0; i < OPLUS_SUB_BTB_MAX - 1; i++)
			chg_info(" valid_temp[%d] = %d", i, chip->sub_btb_valid_adc[i]);
	}
...
static int mtk_charger_probe(struct platform_device *pdev)
{
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	oplus_gauge_sub_btb_parse_dt(info, &pdev->dev);
	pinfo = info;
	...
	pinfo->master_cp_chan = devm_iio_channel_get(&pdev->dev, "master_cp_temp");
	if (IS_ERR(pinfo->master_cp_chan)) {
		chg_err("Couldn't get master_cp_chan...\n");
		pinfo->master_cp_chan = NULL;
	}

	pinfo->slave_cp_chan = devm_iio_channel_get(&pdev->dev, "slave_cp_temp");
	...
	if (rc)
		chg_err("register thermal fail");
#endif
	oplus_mt6379_set_chging_term_disable(info);
	oplus_mms_wait_topic("wireless", oplus_mt6379_subscribe_wls_topic, info);
reg_ic_err:
#endif
