...
bool ux_need_up_migration(struct task_struct *p, struct rq *rq)
{
	 ...
	 */
	if (topology_cluster_id(cpu_of(rq)))
		return false;
	...
	trace_printk("OPLUS_LB_TICK[%d]: task=%s$%d, mask=[%*pbl], "
		...
		cpu_of(rq), topology_cluster_id(cpu_of(rq)),
			...
			int prev_cpu, enum migr_type type, bool latency_sensive)
{
	...
	int prev_cls = topology_cluster_id(prev_cpu);
	int order_idx = -1;
	int walk_cnt = -1, idx;
	int cpu = -1;
	bool ret;
...
{
	return topology_cluster_id(new_cpu) == topology_cluster_id(prev_cpu);
}
...
static noinline bool oplus_migrate_running_ux(void *data, struct rq *rq)
{
	...
	trace_printk("OPLUS_LB_TICK[%d]: curr=%s$%d, "
		...
		topology_cluster_id(new_cpu) > topology_cluster_id(prev_cpu));
#endif
...
bool rt_need_up_migration(struct task_struct *p, struct rq *rq)
{
	...
	/*
	 * We only allow rt to migrate from silver to gold or
	 * prime core.
	 */
	if (topology_cluster_id(cpu_of(rq)))
		return false;
...
static int oplus_get_cluster_util(int cpu)
{
	...
	int order_idx = topology_cluster_id(cpu);
...
bool has_enough_capacity(int this_cpu)
{
	...
	oplus_loadbalance_systrace_print(OPLUS_LB_SYSTRACE_PID,
					"newidle_cls_util",
					topology_cluster_id(MIN_CPU),
					oplus_get_cluster_util(MIN_CPU));
	oplus_loadbalance_systrace_print(OPLUS_LB_SYSTRACE_PID,
					"newidle_cls_util",
					topology_cluster_id(this_cpu),
					oplus_get_cluster_util(this_cpu));
	oplus_loadbalance_systrace_print(OPLUS_LB_SYSTRACE_PID,
					"newidle_cls_util",
					topology_cluster_id(MAX_CPU),
