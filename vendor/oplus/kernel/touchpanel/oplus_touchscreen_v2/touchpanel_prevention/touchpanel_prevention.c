...
static bool point_edge_degree_check(struct kernel_grip_info *grip_info, struct point_info *points, int index)
{
	...
	uint16_t edge_min_channel = grip_info->corner_point_edge_min_channel_thd;
	uint16_t corner_percent_thd = grip_info->corner_point_edge_percent_thd;
	uint16_t trx_corner_percent_thd = grip_info->top_trx_corner_point_edge_percent_thd;
	uint16_t x_coupling_result = 0, y_coupling_result = 0;
...
static const struct key_addr key_addr_arrays[] = {
	...
	/*add for curved_screen_v4.3  recling mode begin*/
...
static int kernel_grip_init_v4_3(struct kernel_grip_info *grip_info, struct device *dev)
{
	...
	if (ret) {
		...
		grip_info->uniform_make_up_point_v2_support = 1;
		TPD_INFO("point_corner_er_min_limit_check using default.\n");
	} else {
