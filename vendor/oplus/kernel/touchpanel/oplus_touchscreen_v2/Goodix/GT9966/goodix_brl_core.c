...
static int get_now_temp(struct chip_data_brl *chip_info)
{
	...
	if (ts->is_suspended) {
		TPD_INFO("GT:%s :ERR!ts->is_suspended\n", __func__);
		goto OUT_ERR;
	}
	...
	if (ts->temperature_detect_shellback_support) {
		...
		if (rc < 0) {
			TPD_INFO("GT:%s:ERR!can't get skin temp,rc=%d\n", __func__, rc);
			goto OUT_ERR;
		}
	}
	result = result / 1000;
...
static int goodix_get_vendor(void *chip_data, struct panel_info *panel_data)
{
	...
	strncpy(panel_data->manufacture_info.manufacture, manu_temp,
		MAX_DEVICE_MANU_LENGTH);
#endif
	TPD_INFO("GT:chip_info->tp_type = %d, panel_data->fw_name = %s\n",
...
static int convert_ic_info(struct goodix_ic_info *info, const u8 *data)
{
	...
	TPD_INFO("GT:frame_data_addr:               0X%4x\n", misc->frame_data_addr);
	TPD_INFO("GT:touch_data_addr:               0X%4x\n", misc->touch_data_addr);
	return 0;
}
				      ...
				      struct panel_info *panel_data)
{
	...
	if (panel_data->manufacture_info.version) {
		...
		if (strlen(chip_info->get_new_trx_name) > 0) {
			sprintf(dev_version, "%02x%02x",
				fw_ver_num & 0xff,
				panel_data->tp_fw);
			TPD_INFO("GT:%s,many panel[tpFw:%2x][fwVersion:0x%x][version:%s]\n",
...
static enum hrtimer_restart goodix_check_hrtimer(struct hrtimer *timer)
{
	...
	TPD_DEBUG("GT:%s: check_hrtimer_over:%d,check_id:%d,check_start:%d, palm:%d\n", \
		__func__,
...
OUT:
	TPD_DEBUG("GT:%s:%s->id[%d]check_id[%d]timer_over[%d]status[%d]touch_num[%d]ret[%d]palm[%d]",
		__func__,
			...
			struct point_info *points, int max_num)
{
	...
	for (true_num = 0; true_num < touch_num; true_num++) {
		...
		/* edge data */
		if (chip_info->hardware_trx_direction == TX_LEFT_RIGHT) {
		...
		} else if (chip_info->hardware_trx_direction == TX_DOWN_UP) {
			points[id].tx_press = DATA_CHANGE(ew_data[0]); /* 0X102F6 */
			points[id].rx_press = DATA_CHANGE(ew_data[1]); /* 0X102F7 */
			points[id].tx_er	= DATA_CHANGE(ew_data[3]); /* 0X102F8 */
			points[id].rx_er	= DATA_CHANGE(ew_data[2]); /* 0X102F9 */
		}
		/* touch status */
		...
		TPD_DEBUG("GT:[TP]:DOWN:%d:%s:(%6d,%6d).tp[%2d].rp[%2d].te[%2d].re[%2d]\n",
			id,
			points[id].x > (chip_info->max_y / 2) ? "right" : "left",
...
static void goodix_gesture_coordiate(struct chip_data_brl *chip_info, struct gesture_info *gesture)
{
	...
	TPD_INFO("GT:%s:(X,Y)=>kernel[%5d,%5d] user[%4d,%4d]",
		__func__,
...
static void goodix_get_health_info(void *chip_data, struct monitor_data *mon_data)
{
	...
	if (ret < 0) {
		TPD_INFO("GT:%s: read debug log data i2c faild\n", __func__);
		return;
	}
	...
	if (health_info->shield_freq != 0
		...
		if (!freq_str) {
			TPD_INFO("GT:freq_str kzalloc failed.\n");
		} else {
			if (health_info->charger_mode) {
	...
	if (ret < 0) {
		TPD_INFO("GT:%s: clear debug log data i2c faild\n", __func__);
	}
...
static int goodix_read_coord_position(void *chip_data, char *page, size_t str)
{
	...
	TPD_INFO("GT:%s: start to goodix_read_coord_position, str:%d\n", __func__, str);
...
static void set_new_offect(struct chip_data_brl *chip_info, int new_offect)
{
	...
	TPD_INFO("GT:%s:new motor para:[%d,%d]~[%d,%d][offect:%d]\n", __func__,
		chip_info->motor_min_min, chip_info->motor_min_max,
		chip_info->motor_max_min, chip_info->motor_max_max,
		new_offect);
}
...
static int goodix_set_coord_position(void *chip_data, int position)
{
	...
	if (position >= chip_info->motor_min_min &&
		...
		if (chip_info->motor_max_limit == false) {
			...
			goodix_check_reg(chip_info, GOODIX_CMD_REG, 8);
			chip_info->motor_max_limit = true;
		TPD_INFO("GT:%s: RUNING position:%d\n", __func__, chip_info->motor_get_coord);
		}
...
static int goodix_mode_switch(void *chip_data, work_mode mode, int flag)
{
	...
	case MODE_PEN_SCAN:
		...
		if (ret < 0) {
			TPD_INFO("GT:%s: enable pen mode : %d failed\n", __func__, flag);
		}
	...
	case MODE_PEN_CTL:
		...
		if (ret < 0) {
			TPD_INFO("GT:%s: enable pen control : %d failed\n", __func__, flag);
		}
		break;
	default:
...
static void goodix_enable_gesture_mask(void *chip_data, uint32_t enable)
{
	...
	TPD_INFO("GT:%s: abandon gesture_type:0x%08X\n",
		__func__, chip_info->gesture_type);
}
...
static void goodix_check_bit_chg(struct chip_data_brl *chip_info, unsigned int bit)
{
	...
	TPD_DEBUG("GT: %s-->[old:%d]-[now:%d]-[chg:%d]\n",
		__func__,
...
static void goodix_state_verify(struct chip_data_brl *chip_info)
{
	...
	if (chip_info->check_chg_state &&
		...
		if (BIT_CHK(chip_info->check_now_state, GAME_MODE_ENABLE)) {
			...
			TPD_INFO("GT: %s->game off->must enable pen and pen ctl:%d\n", __func__, chip_info->pen_ctl_para);
...
static int goodix_send_temperature(void *chip_data, int temp, bool normal_mode)
{
	...
	if (normal_mode == true) {
		...
		TPD_DEBUG("GT:%s [cnt:%d][temp:%d][def_temp0:%d,def_temp1:%d]", __func__,
...
static void gt_fw_status_in_differ(struct chip_data_brl *chip_info, bool on)
{
	...
	TPD_INFO("GT:%s -> %s fw mode", __func__, on ? "enable" : "disable");

	if (on) {
		if(chip_info->ic_info.misc.frame_data_addr == DATA_SYNC_ALGOLIB_ADDR) {
			TPD_INFO("GT:algolab fw,send cmd\n");
			temp_cmd.len     = 7;
			temp_cmd.cmd     = 0x90;
			temp_cmd.data[0] = 0x42;
			temp_cmd.data[1] = 0x01;
			ret = brl_send_cmd(chip_info, &temp_cmd);
		} else {
			ret = goodix_send_cmd_simple(chip_info, GTP_CMD_FW_STATUS, GTP_CMD_FW_STATUS_ON);
		}
	} else {
		if(chip_info->ic_info.misc.frame_data_addr == DATA_SYNC_ALGOLIB_ADDR) {
			ret = goodix_send_cmd_simple(chip_info, GTP_CMD_FW_STATUS, 0);
			msleep(50);
			ret = goodix_send_cmd_simple(chip_info, GTP_CMD_QUIT_IDLE, 0);
		} else {
			ret = goodix_send_cmd_simple(chip_info, GTP_CMD_FW_STATUS, GTP_CMD_FW_STATUS_OFF);
			msleep(50);
			ret = goodix_send_cmd_simple(chip_info, GTP_CMD_QUIT_IDLE, 0);
		}
	}
...
static int gt_transfer_check_data(struct chip_data_brl *chip_info,
	struct goodix_ic_info *ic_info, u8 *kernel_buf, u8 clear_state,
	debug_type debug_type, u32 data_addr)
{
	int ret = 0;
	int i = 0;

	if (debug_type == GTP_FW_STATUS) {
		TPD_INFO("GT:%s-> now open fw_status mode", __func__);
		gt_fw_status_in_differ(chip_info, true);
	} else {
		if(ic_info->misc.frame_data_addr == DATA_SYNC_ALGOLIB_ADDR) {
			TPD_INFO("GT:%s change data mode\n", __func__);
			ret = goodix_send_cmd_simple(chip_info, GTP_CMD_FW_STATUS, GTP_CMD_DEBUG);
		} else {
			ret = goodix_send_cmd_simple(chip_info, GTP_CMD_RAWDATA, 0);
		}
	}

	msleep(GTP_CMD_DELAY);
	ret = goodix_reg_write(chip_info, data_addr, &clear_state, 1);
	msleep(GTP_CMD_DELAY);
	while (i++ < GTP_CMD_DELAY) {
		ret = goodix_reg_read(chip_info, data_addr, kernel_buf, 1);
		TPD_INFO("GT[ret:%d][kernel_buf:%d][addr:0x%4x]\n", ret, kernel_buf[0], data_addr);
		if (!ret && (kernel_buf[0] & 0x80)) {
			TPD_INFO("GT:Data ready OK\n");
			break;
		}
		msleep(GTP_CMD_DELAY);
	}
	...
	if (i >= 10) {
		TPD_INFO("GT:data not ready, quit!\n");
		ret = -1;
	}
...
static void gt_transfer_get_data(struct seq_file *s,
	struct chip_data_brl *chip_info,
	u8 *kernel_buf, int addr, int tx_num, int rx_num, s16 data,
				   ...
				   void *chip_data, debug_type debug_type)
{
	...
	if (!tx_num || !rx_num) {
		TPD_INFO("GT:%s: error invalid tx %d or rx %d num\n",
			 __func__, tx_num, rx_num);
		return;
	}

	TPD_INFO("GT:%s: detect (tx,rx)->(%2d,%2d)\n",
			 __func__, tx_num, rx_num);
	...
	switch (debug_type) {
	case GTP_RAWDATA:
		TPD_INFO("GT:%s-> mode:GTP_RAWDATA", __func__);
		addr = ic_info->misc.mutual_rawdata_addr;
	break;
	case GTP_DIFFDATA:
		TPD_INFO("GT:%s-> mode:GTP_DIFFDATA", __func__);
		addr = ic_info->misc.mutual_diffdata_addr;
	break;
	case GTP_FW_STATUS:
		TPD_INFO("GT:%s-> mode:GTP_FW_STATUS", __func__);
		addr = ic_info->misc.mutual_diffdata_addr;
		tx_num += DATA_DEGBU_FW_TX_OFFECT;
	break;
	case GTP_BASEDATA:
		TPD_INFO("GT:%s-> mode:GTP_BASEDATA", __func__);
		addr = ic_info->misc.mutual_refdata_addr;
	break;
	case GTP_NORMAL_LIZE:
		TPD_INFO("GT:%s-> mode:GTP_NORMAL_LIZE", __func__);
		addr = GTP_TEST_NORMALIZE;
	break;
	default:
		TPD_INFO("GT:%s-> mode:default mode", __func__);
		addr = ic_info->misc.mutual_refdata_addr;
	break;
	}

	ret = gt_transfer_check_data(chip_info, ic_info,
	            kernel_buf, clear_state, debug_type, data_addr);
	if (ret < 0) {
		TPD_INFO("GT:data not ready, quit!\n");
		goto read_data_exit;
	}
	...
	gt_transfer_get_data(s, chip_info, kernel_buf, addr,
	                     tx_num, rx_num, data, debug_type);

read_data_exit:
	if (debug_type == GTP_FW_STATUS) {
		TPD_INFO("GT:%s-> now close fw_status mode", __func__);
		gt_fw_status_in_differ(chip_info, false);
	}
...
{
	TPD_INFO("GT:%s: start\n", __func__);
	goodix_debug_info_read(s, chip_data, GTP_FW_STATUS);
	goodix_debug_info_read(s, chip_data, GTP_DIFFDATA);
	goodix_debug_info_read(s, chip_data, GTP_BASEDATA);
				       ...
				       struct test_item_info *p_test_item_info)
{
	...
	if (cd->test_cfg.length > 0) {
		...
		if (ret < 0) {
			TPD_INFO("GT:send test config failed\n");
			return -EINVAL;
		}
	}

	if(cd->ic_info.misc.frame_data_addr == DATA_SYNC_ALGOLIB_ADDR) {
		TPD_INFO("GT:algolab fw,change addr with 0x%4x\n", cd->ic_info.misc.frame_data_addr);
		temp_cmd.data[0] = 0x42;
		coor_addr        = DATA_SYNC_ALGOLIB_ADDR;
	} else {
	...
	if (ret < 0) {
		TPD_INFO("GT:%s Enter rawdata mode failed\n", __func__);
		return ret;
	}
	...
	while (--discard_cnt) {
		msleep(20);
		ret = goodix_reg_write(cd, coor_addr, &status, 1);
	}
	...
	if (retry < 0) {
		TPD_INFO("GT:rawdata is not ready val:0x%02x exit", status);
		return -EINVAL;
	}
				...
				struct test_item_info *p_test_item_info)
{
	...
	if (retry < 0) {
		TPD_INFO("GT:%s: switch clk test mode failed, sta[%x] ack[%x]\n",
				__func__, rcv_buf[0], rcv_buf[1]);
		seq_printf(s, "switch clk test mode failed, sta[%x] ack[%x]\n", rcv_buf[0], rcv_buf[1]);
		err_cnt++;
		goto exit;
	}

	if (pen_osc_clk == EXT_OSC_FREQ_64K) {
		clk_parm.gio = 14;
		clk_parm.div = 1;
		clk_parm.gio_set = 0;
		clk_parm.en = 1;
	...
	if (retry < 0) {
		TPD_INFO("GT:%s: wait clk test result failed, sta[%x] ack[%x]\n",
				__func__, rcv_buf[0], rcv_buf[1]);
		seq_printf(s, "wait clk test result failed, sta[%x] ack[%x]\n", rcv_buf[0], rcv_buf[1]);
		err_cnt++;
		goto exit;
	}
	...
	if (checksum_cmp(rcv_buf, sizeof(rcv_buf), CHECKSUM_MODE_U8_LE)) {
		TPD_INFO("GT:%s: clk test result checksum error, [%*ph]\n",
				__func__, (int)sizeof(rcv_buf), rcv_buf);
		seq_printf(s, "clk test result checksum error, [%*ph]\n", (int)sizeof(rcv_buf), rcv_buf);
		err_cnt++;
		goto exit;
	}
	...
	if (pen_osc_clk == EXT_OSC_FREQ_64K)
		cal_freq = clk_in_cnt * INTER_CLK_FREQ * 8 / clk_osc_cnt;
	else
		cal_freq = clk_in_cnt * INTER_CLK_FREQ / clk_osc_cnt;

	TPD_INFO("GT:%s: clk_in_cnt:%lld clk_osc_cnt:%lld cal_freq:%lld\n",
	...
	if (freq_delta * 100 / pen_osc_clk > 2) {
		seq_printf(s, "OSC NG!! clk_in_cnt:%lld clk_osc_cnt:%lld cal_freq:%lld\n",
			clk_in_cnt, clk_osc_cnt, cal_freq);
		err_cnt++;
		TPD_INFO("GT:%s: osc clk test failed\n", __func__);
	} else {
		TPD_INFO("GT:%s: osc clk test pass\n", __func__);
		ret = 0;
	}

exit:
	if (err_cnt > 0) {
		ts_test->test_result[TYPE_TEST6] = GTP_TEST_NG;
		return RESULT_ERR;
	}
				...
				struct test_item_info *p_test_item_info)
{
	...
	if (p_test_item_info->para_num && p_test_item_info->p_buffer[0]) {
		...
		TPD_INFO("GT:noise_threshold:%d\n", noise_threshold);
	} else {
		TPD_INFO("GT:skip noisedata test\n");
		return 0;
	}
	...
	for (i = 0; i < ts_test->noisedata.size; i++) {
		...
		if (val > noise_threshold) {
			TPD_INFO("GT:noisedata[%d]:%d > noise_threshold[%d]\n", i, val, noise_threshold);
				...
				struct test_item_info *p_test_item_info)
{
	...
	if (p_test_item_info->para_num && p_test_item_info->p_buffer[0]) {
		...
		if (p_test_item_info->item_limit_type == LIMIT_TYPE_TX_RX_DATA) {
			...
			TPD_INFO("GT:item: %d get_test_item_info fail\n", LIMIT_TYPE_TX_RX_DATA);
			return RESULT_ERR;
		}
	} else {
		TPD_INFO("GT:skip rawdata test\n");
		return 0;
	}
				...
				struct test_item_info *p_test_item_info)
{
	...
	if (p_test_item_info->para_num && p_test_item_info->p_buffer[0]) {
		...
		TPD_INFO("GT:skip deltadata test\n");
		return 0;
	}
	...
	for (i = 0; i < ts_test->deltadata.size; i++) {
		...
		if (val > delta_threshold[i]) {
			TPD_INFO("GT:deltadata[%d] out of threshold[%d]\n", val, delta_threshold[i]);
				...
				struct test_item_info *p_test_item_info)
{
	...
	if (p_test_item_info->para_num && p_test_item_info->p_buffer[0]) {
		...
		TPD_INFO("GT:skip self_rawdata test\n");
		return 0;
	}
	...
	for (i = 0; i < ts_test->self_rawdata.size; i++) {
		...
		if (val > max_limit[i] || val < min_limit[i]) {
			TPD_INFO("GT:self_rawdata[%d] out of threshold[%d,%d]\n", val, min_limit[i], max_limit[i]);
...
static int brl_short_test_prepare(struct chip_data_brl *cd)
{
	...
	TPD_INFO("GT:short test prepare IN\n");
...
resend_cmd:
	...
	if (ret < 0) {
		TPD_INFO("GT:send test mode failed\n");
		return ret;
	}
	...
	while (retry--) {
		...
		if (!ret && status == SHORT_TEST_RUN_FLAG)
			return 0;
		TPD_INFO("GT:short_mode_status=0x%02x ret=%d\n", status, ret);
	}
...
static u32 map_die2pin(struct ts_test_params *test_params, u32 chn_num)
{
	...
	if (i >= test_params->max_drv_num)
		TPD_INFO("GT:Faild found corrresponding channel num:%d\n", chn_num);
	else
        ...
        u8 short_ch_num)
{
	...
	for (i = 0; i < short_ch_num; i++) {
		...
		if (ret < 0) {
			TPD_INFO("GT:Failed read Drv-to-Drv short rawdata\n");
			err = -EINVAL;
			break;
		}

		if (checksum_cmp(data_buf, size, CHECKSUM_MODE_U8_LE)) {
			TPD_INFO("GT:Drv-to-Drv adc data checksum error\n");
			err = -EINVAL;
			break;
		}
		...
		if (short_die_num >= max_drv_num) {
			TPD_INFO("GT:invalid short pad num:%d\n",
				short_die_num + max_sen_num);
			continue;
		}
		...
		if (self_capdata == 0xffff || self_capdata == 0) {
			TPD_INFO("GT:invalid self_capdata:0x%x\n", self_capdata);
			continue;
		}
		...
		for (j = short_die_num + 1; j < max_drv_num; j++) {
			...
			if (short_r < r_threshold) {
				...
				if (master_pin_num == 0xFF || slave_pin_num == 0xFF) {
					TPD_INFO("GT:WARNNING invalid pin\n");
					continue;
				}
				TPD_INFO("GT:short circut:R=%dK,R_Threshold=%dK",
							short_r, r_threshold);
				TPD_INFO("GT:%s%d--%s%d shortcircut\n",
        ...
        u8 short_ch_num)
{
	...
	for (i = 0; i < short_ch_num; i++) {
		ret = goodix_reg_read(cd, data_reg, data_buf, size);
		if (ret) {
			TPD_INFO("GT:Failed read Sen-to-Sen short rawdata\n");
			err = -EINVAL;
			break;
		}

		if (checksum_cmp(data_buf, size, CHECKSUM_MODE_U8_LE)) {
			TPD_INFO("GT:Sen-to-Sen adc data checksum error\n");
			err = -EINVAL;
			break;
		}
        ...
        u8 short_ch_num)
{
	...
	for (i = 0; i < short_ch_num; i++) {
		ret = goodix_reg_read(cd, data_reg, data_buf, size);
		if (ret) {
			TPD_INFO("GT:Failed read Drv-to-Sen short rawdata\n");
			err = -EINVAL;
			break;
		}

		if (checksum_cmp(data_buf, size, CHECKSUM_MODE_U8_LE)) {
			TPD_INFO("GT:Drv-to-Sen adc data checksum error\n");
			err = -EINVAL;
			break;
		}
        ...
        u16 adc_signal, u32 pos)
{
	...
	if (r < r_th) {
		...
		TPD_INFO("GT:%s%d shortcircut to %s,R=%ldK,R_Threshold=%dK\n",
...
static int gdix_check_gndvdd_shortcircut(struct chip_data_brl *cd)
{
	...
	ret = goodix_reg_read(cd, data_reg, data_buf, size);
	if (ret < 0) {
		TPD_INFO("GT:Failed read to-gnd rawdata\n");
		err = -EINVAL;
		goto err_out;
	}

	if (checksum_cmp(data_buf, size, CHECKSUM_MODE_U8_LE)) {
		TPD_INFO("GT:diff code checksum error\n");
		err = -EINVAL;
		goto err_out;
	}
	...
	for (i = 0; i < max_drv_num + max_sen_num; i++) {
		...
		ret = gdix_check_resistance_to_gnd(cd,
					adc_signal, i);
		if (ret != 0) {
			TPD_INFO("GT:Resistance to-gnd/vdd short\n");
			err = ret;
		}
	}
...
static int brl_shortcircut_analysis(struct chip_data_brl *cd)
{
	...
	if (ret < 0) {
		TPD_INFO("GT:%s: Read TEST_RESULT_REG failed\n", __func__);
		return ret;
	}

	if (checksum_cmp((u8 *)&test_result, sizeof(test_result),
		CHECKSUM_MODE_U8_LE)) {
		TPD_INFO("GT:%s: shrot result checksum err\n", __func__);
		return -EINVAL;
	}

	TPD_INFO("GT:%s :short flag 0x%02x, drv&drv:%d, sen&sen:%d, drv&sen:%d, drv/GNDVDD:%d, sen/GNDVDD:%d\n",
		__func__,
	...
	TPD_INFO("GT:%s : >>>>> short check return 0x%x\n", __func__, err);
				...
				struct test_item_info *p_test_item_info)
{
	...
	if (p_test_item_info->para_num && p_test_item_info->p_buffer[0]) {
		...
		TPD_INFO("GT:%s: skip shortcircuit test\n", __func__);
		return 0;
	}

	ret = brl_short_test_prepare(cd);
	if (ret < 0) {
		TPD_INFO("GT:Failed enter short test mode\n");
		return RESULT_ERR;
	}
	...
	if (ret < 0) {
		TPD_INFO("GT:Failed to get test_time, default %dms\n", DEFAULT_TEST_TIME_MS);
		test_time = DEFAULT_TEST_TIME_MS;
	} else {
		if (test_time > MAX_TEST_TIME_MS) {
			TPD_INFO("GT:test time too long %d > %d\n",
				test_time, MAX_TEST_TIME_MS);
			test_time = MAX_TEST_TIME_MS;
		}
		TPD_INFO("GT:get test time %dms\n", test_time);
	}
	...
	if (retry < 0) {
		TPD_INFO("GT:short test failed, status:0x%02x\n", status);
		return RESULT_ERR;
	}

	/* start analysis short result */
	TPD_INFO("GT:short_test finished, start analysis\n");
				 ...
				 struct test_item_info *p_test_item_info)
{
	...
	for (i = 0; i < MAX_TEST_ITEMS; i++) {
		...
		TPD_INFO("GT:test_result_info %s: %s\n", test_item_name[i],
				...
				struct test_item_info *p_test_item_info)
{
	...
	if (cd->normal_cfg.length > 0) {
		...
		if (ret < 0)
			TPD_INFO("GT:send normal config failed\n");
	}
...
static void init_motor_para(struct chip_data_brl *chip_info)
{
	...
	TPD_INFO("GT:motor parse OK:[%3d~%3d,%3d~%3d],virtual_origin:%d,k:%d,offect:%d,prevent:%d",
			chip_info->motor_min_min, chip_info->motor_min_max,
			chip_info->motor_max_min, chip_info->motor_max_max,
...
static void change_trx_config(struct chip_data_brl *chip_info)
{
	char *ret = NULL;
	ret = strstr(chip_info->ts->panel_data.fw_name, chip_info->get_new_trx_name);
	if (ret == NULL) {
		TPD_INFO("GT:%s:[%s]not[%s]config, not to change trx", __func__,
			chip_info->ts->panel_data.fw_name, chip_info->get_new_trx_name);
	} else {
		chip_info->ts->hw_res.tx_num = chip_info->get_new_trx[0];
		chip_info->ts->hw_res.rx_num = chip_info->get_new_trx[1];
		chip_info->panel_type = SAMSUNG_PANEL;
		TPD_INFO("GT:%s:[%s]match[%s]config,need to change trx(%2d,%2d)", __func__,
			chip_info->ts->panel_data.fw_name, chip_info->get_new_trx_name,
			chip_info->ts->hw_res.tx_num, chip_info->ts->hw_res.rx_num);
		if (chip_info->pen_support) {
			chip_info->ts->hw_res.pen_tx_num = chip_info->ts->hw_res.tx_num;
			chip_info->ts->hw_res.pen_rx_num = chip_info->ts->hw_res.rx_num;
			TPD_INFO("GT:%s:pen need update trx(%2d,%2d),too", __func__,
			chip_info->ts->hw_res.pen_tx_num, chip_info->ts->hw_res.pen_rx_num);
		}
	}
}

static void change_clk_config(struct chip_data_brl *chip_info)
{
	char *ret   = NULL;
	char *strv  = NULL;
	char *get_new_clk_name = NULL;

	strv = chip_info->get_new_clk_name;
	get_new_clk_name = strsep(&strv, CLK_NAME);

	ret = strstr(chip_info->ts->panel_data.fw_name, get_new_clk_name);
	if (ret == NULL) {
		TPD_INFO("GT:%s:[%s]not[%s]config, not to change clk", __func__,
			chip_info->ts->panel_data.fw_name, get_new_clk_name);
	} else {
		if (chip_info->get_new_clk[0] == DEF_OSC_FREQ_16K) {
			chip_info->pen_osc_frequency = DEF_OSC_FREQ_16K;
			TPD_INFO("GT:%s:success change clk to %dmHz\n", __func__, chip_info->pen_osc_frequency);
		}
	}
}
...
static void init_goodix_chip_dts(struct device *dev, void *chip_data)
{
	...
	const char *new_trx_name = NULL;
	const char *new_clk_name = NULL;
	...
	if (!chip_np) {
		TPD_INFO("GT:%s: fail get child node for gt9966", __func__);
		return;
	} else {
		TPD_INFO("GT:%s: success get child node for gt9966", __func__);
	}
	...
	if (rc) {
		TPD_INFO("GT:%s: fail get gt9966_get_motor_coord %d\n", __func__, rc);
	} else {
		...
		if (0 == cnt) {
			TPD_INFO("GT:%s: invalid data!! stop to parse\n", __func__);
			goto OUT_ERR;
		}
	...
	if (rc) {
		TPD_INFO("GT:%s: fail get gt9966_dynamic_limit %d\n", __func__, rc);
	} else {
	...
	if (chip_info->gt_temperature[0] > 0 && chip_info->gt_temperature[1] > 0) {
		if (of_property_read_u32_array(chip_np, "thermal_detect_support", temp_array, GOODIX_MAX_TEMP)) {
			TPD_INFO("GT:%s: not to change temperature zone\n", __func__);
		} else {
			chip_info->gt_temperature[0] = temp_array[0];
			chip_info->gt_temperature[1] = temp_array[1];
			TPD_INFO("GT:%s: change temperature pass -> [%u],[%u]\n",
				__func__, chip_info->gt_temperature[0], chip_info->gt_temperature[1]);
		}
	}

	/* maybe panel was changed, so need reset some config parameter */
	rc = of_property_read_string(chip_np, "need_change_trx_name", &new_trx_name);
	if (rc) {
		TPD_INFO("GT:%s: the need_change_trx_name is not found !\n", __func__);
	} else {
		TPD_INFO("GT:%s: new trx_name:%s\n", __func__, new_trx_name);
		if (strlen(new_trx_name) >= sizeof(chip_info->get_new_trx_name)) {
			TPD_INFO("GT:%s: new_trx_name length out of bounds\n", __func__);
		} else {
			memset(chip_info->get_new_trx_name, 0, sizeof(chip_info->get_new_trx_name));
			rc = of_property_read_u32_array(chip_np, new_trx_name, chip_info->get_new_trx, 2);
			strncpy(chip_info->get_new_trx_name, new_trx_name, strlen(new_trx_name));
			if (rc) {
				...
				TPD_INFO("GT:%s: new CHIP NAME[%s,%lu]:(tx,rx)=(%2d,%2d)\n",
						...
						chip_info->get_new_trx[1]);
			}
		}
	}

	rc = of_property_read_string(chip_np, "need_change_clk_name", &new_clk_name);
	if (rc) {
		TPD_INFO("GT:%s: the need_change_clk_name is not found !\n", __func__);
	} else {
		TPD_INFO("GT:%s: new clk_name:%s\n", __func__, new_clk_name);
		if (strlen(new_clk_name) >= sizeof(chip_info->get_new_clk_name)) {
			TPD_INFO("GT:%s: new_clk_name length out of bounds\n", __func__);
		} else {
			memset(chip_info->get_new_clk_name, 0, sizeof(chip_info->get_new_clk_name));
			rc = of_property_read_u32_array(chip_np, new_clk_name, chip_info->get_new_clk, 1);
			strncpy(chip_info->get_new_clk_name, new_clk_name, strlen(new_clk_name));
			if (rc) {
				...
				TPD_INFO("GT:%s: new CHIP NAME[%s,%lu]:CLK=%d\n",
						...
						chip_info->get_new_clk[0]);
			}
		}
	}

	if (of_property_read_u32(chip_np, "hardware_trx_direction", &ret) < 0) {
		chip_info->hardware_trx_direction = TX_LEFT_RIGHT;
		TPD_INFO("GT:%s:hardware_trx_direction default:%u\n", __func__, chip_info->hardware_trx_direction);
	} else {
		chip_info->hardware_trx_direction = TX_DOWN_UP;
		TPD_INFO("GT:%s:change hardware trx direction :%u\n", __func__, chip_info->hardware_trx_direction);
	}
...
static int goodix_gt9966_ts_probe(struct spi_device *spi)
{
	...
	chip_info->pen_max_x = 22020;
	chip_info->pen_min_y = 660;
	chip_info->pen_max_y = 23740;
	chip_info->ts = ts;
	...
	if(chip_info->pen_support) {
		TPD_INFO("GT:%s, start check in pen hrtimer\n", __func__);
	...
	if (!!strlen(chip_info->get_new_trx_name)) {
		change_trx_config(chip_info);
	}

	if (!!strlen(chip_info->get_new_clk_name)) {
		change_clk_config(chip_info);
	}
	...
	if (chip_info->ts->temperature_detect_support ||
		...
		chip_info->temp_recorder_cnt = CNT_CLR;
		TPD_INFO("GT:%s:gt_temp_para set default control->[%u][%u]\n",
			__func__, chip_info->gt_temperature[0], chip_info->gt_temperature[1]);
	} else {
		...
		chip_info->temp_recorder_cnt = CNT_CLR;
		TPD_INFO("GT:%s:failed set temperature control->[%u][%u]\n",
			__func__, chip_info->gt_temperature[0], chip_info->gt_temperature[1]);
	}
...
#endif
{
	...
	if (chip_info->pen_healthinfo_node == true) {
		TPD_INFO("GT:%s need remove healthinfo node\n", __func__);
