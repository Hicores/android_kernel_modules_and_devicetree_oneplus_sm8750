...
static void pogo_keyboard_connect_send_nfc_uevent(void)
{
    ...
    ret = kobject_uevent_env(&pogo_keyboard_client->uevent_nfc_dev->kobj, KOBJ_CHANGE, envp);
    if (ret) {
        kb_err("%s: kobject_uevent_fail, ret = %d", __func__, ret);
    }
...
static int pogo_keyboard_init_device_uevent(void)
{
    ...
    pogo_keyboard_client->uevent_class = class_create("pogopin");
#else
...
static int pogo_keyboard_init_device_uevent_nfc(void)
{
    int status = 0;
    dev_t devt;

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0))
    pogo_keyboard_client->uevent_nfc_class = class_create("pogopin_nfc");
#else
    pogo_keyboard_client->uevent_nfc_class = class_create(THIS_MODULE, "pogopin_nfc");
#endif
    if (pogo_keyboard_client->uevent_nfc_class == NULL) {
        return EINVAL;
    }

    status = alloc_chrdev_region(&devt, 0, 1, "keyboard_nfc");
    if (status < 0) {
        class_destroy(pogo_keyboard_client->uevent_nfc_class);
        return status;
    }

    pogo_keyboard_client->uevent_nfc_dev = device_create(pogo_keyboard_client->uevent_nfc_class, NULL, devt, NULL, "%s", KEYBOARD_NFC_NAME);
    if (pogo_keyboard_client->uevent_nfc_dev == NULL) {
        unregister_chrdev_region(devt, 1);
        class_destroy(pogo_keyboard_client->uevent_nfc_class);
        return EINVAL;
    }

    pogo_keyboard_client->uevent_nfc_dev->devt = devt;

    return 0;
}

static void pogo_keyboard_deinit_device_uevent_nfc(void)
{
    device_destroy(pogo_keyboard_client->uevent_nfc_class, pogo_keyboard_client->uevent_nfc_dev->devt);
    unregister_chrdev_region(pogo_keyboard_client->uevent_nfc_dev->devt, 1);
    class_destroy(pogo_keyboard_client->uevent_nfc_class);
}
...
int pogo_keyboard_plat_probe(struct platform_device *device)
{
    ...
    ret = pogo_keyboard_init_device_uevent_nfc();
    if (ret != 0) {
...
err_init_proc:
    pogo_keyboard_deinit_device_uevent_nfc();
err_init_uevent_nfc:
